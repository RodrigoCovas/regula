...
Update 2026-09-03 (#68, ADR-0014): the agreement comparison this ADR kept is retired — the rejected first option above happened for the metric anyway, because with the produced side rated, comparing it against the operator's ratings is a category error between two different axes (centrality to the Scenario versus centrality to the Answer), not a measurement of substance. The decision's substance survives untouched: Citation strength stays the Summarizer's rated feature, on the Citations and in the audit dump, its rubric calibrated against the whole cited set (#67); a partial Summarizer pass now simply ships fewer rated provisions, with nothing to degrade. The operator's ratings keep their one remaining role — weighting eval recall on the expected side (ADR-0010).

Update 2026-09-06 (#80): the eval ledger aggregates the produced side's *given* ratings per target (strongest wins) for its spurious/missed accounting display. Nothing is derived — the ratings are read off the artifact's audit dump exactly as the Summarizer shipped them, and no score consumes the aggregation; the retirement above stands.
