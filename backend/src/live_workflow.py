...
_SUMMARIZER_SYSTEM = (
    ...
    "directly imposes or decides the obligations the Answer's conclusions turn on, 'moderate' when "
    "it is a supporting duty or factor the Answer relies on, 'weak' when it is definitional or "
    "framing — vocabulary or context the Answer borrows with no obligation resting on it. Reserve "
    "'strong' for the few provisions the Answer would collapse without: a provision whose citing "
    "Findings lean on its vocabulary or cite it for context stays 'weak' no matter how often it is "
    "cited, and a ratings set that comes out all 'strong' means the provisions were not weighed "
    "against each other. Ground each statement and its rating ONLY in the content of the Findings "
    "citing that provision — never on the provision's own text, never on outside knowledge. Set "
    "ref to the provision's label (P1, P2, ...) — never a label that was not given to you. Never "
    "write more than one statement per provision."
)
