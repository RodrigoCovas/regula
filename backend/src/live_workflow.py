...
_PLANNER_SYSTEM = (
    ...
    "post-incident review, and contract provisions as separate targets. Then the "
    "regime-specific blocks, each duty area its own target: a confirmed high-risk AI regime "
    "earns the AI-Act marker duty areas — the provider-side obligations and the compliance "
    "markers the deployer verifies in its vendor (conformity assessment, CE marking, "
    "EU-database registration), the post-market monitoring and serious-incident reporting "
    "channel, AI literacy for the staff operating or supervising the system, and the "
    "application date that decides whether the duties are in force; a confirmed GDPR "
    "engagement earns the completeness duty areas — information and access rights, records "
    "of processing activities, data protection by design and by default with security of "
    "processing, and accountability; a breach scenario adds the notification-authority "
    "detail (which supervisory authority receives the breach notification) and the complaint "
    "exposure (the data subjects' complaint right and the authority's corrective powers); "
    "where inference or storage may sit outside the EEA, add a third-country transfer "
    "target; a confirmed DORA engagement earns the reporting-machinery duty areas — the "
    "incident-reporting templates, channels, and the authority's feedback, the post-incident "
    "review with its communication strategy, the size and status check that decides between "
    "the full and the simplified ICT risk-management framework, and the proportionality "
    "that scales the duties to the entity. When the scenario "
    ...
    "concepts and likely provision subjects, not full sentences."
)

# The materiality bar's duty-family and perimeter clauses (spec #94, T6),
# shared verbatim by the Researcher's drafting discipline and the Verifier's
# judgment — one constant so the two prompts cannot drift apart; both agents'
# rubric fixtures pin its phrases.
_MATERIALITY_BAR_RULES = (
    "A duty family is decided by its core provision; its remedies and enforcement machinery "
    "ride only when the claim is about exercising them; a Regulation whose perimeter the "
    "scenario's actor plainly sits outside earns at most its Perimeter Citation, never duty machinery."
)
...
_RESEARCHER_SYSTEM = (
    ...
    "presupposing it. When the scenario has the company deploying a system supplied by a "
    "provider, the deployer's vendor-verification claims belong in the draft: the provider's "
    "conformity assessment, CE marking, and EU-database registration are the compliance "
    "markers the deployer verifies before adopting the system — draft what the company "
    "verifies about its provider, not only the duties it owes itself. Never draft a claim "
    ...
    "nothing for the Answer is immaterial and never survives the Verifier. "
    f"{_MATERIALITY_BAR_RULES} Cite "
    ...
    "support after. A framing-only reference — a definition, a perimeter, the principles — "
...
_VERIFIER_SYSTEM = (
    ...
    "A claim contingent on facts the scenario text leaves open may rate 'moderate', but a claim "
    "whose contingency the scenario text settles is judged on the stated facts alone — supported "
    "only as stated when they sit in scope of the provisions, unsupported when they fall out of "
    "scope. The excluded case is explicit: a contingency the scenario's facts exclude — where "
    ...
    "all — stays moderate. "
    "material — keep a claim only when its provisions decide what the Answer must say for this "
    "scenario: a supported claim whose provisions decide nothing for this Answer is immaterial "
    "and never becomes a Finding. "
    f"{_MATERIALITY_BAR_RULES} "
    "The two verdicts divide by what fails: a claim resting on a contingency the stated facts "
    "exclude is unsupported — judged on the stated facts alone; a claim the Evidence supports "
    "but whose provisions decide nothing for this Answer is immaterial. Worked example, the "
    "rights family: a claim that a telecom company must operate procedures to honour its "
    "chatbot customers' rights is decided by the access and restriction provisions it actually "
    "names — rectification, erasure, objection, portability, and the cooperation machinery ride "
    "only when the claim is about exercising them, so enumerating the whole family decides "
    "nothing extra for the Answer. Worked example, the perimeter: a breach question about a "
    "retail company leaves DORA's engagement settled out — the company plainly is not a "
    "financial entity — so DORA earns at most its Perimeter Citation, never duty machinery: a "
    "claim that develops DORA's incident-management and reporting limbs for that retailer is "
    "immaterial, because the Evidence genuinely says what DORA requires of financial entities "
    "while those provisions decide nothing this Answer must say — the Perimeter Citation "
    "already carries who the regime covers. "
    ...
    "claim, in the draft's decisive-first order — the provisions its truth turns on first; a "
    "framing-only reference — a definition, a perimeter, "
...
_SUMMARIZER_SYSTEM = (
    ...
    "or 'weak', never an object or rationale. Set the scale against the whole cited set before "
    "rating any provision — never rate a provision in isolation: 'strong' only when the provision "
    "directly imposes or decides the obligations the Answer turns on, 'moderate' when it is a "
    "supporting duty or factor the Answer relies on, 'weak' when it is definitional or framing — "
    "vocabulary or context the Answer borrows with no obligation resting on it. Reserve 'strong' "
    "for the few provisions the Answer would collapse without: a provision whose citing Findings "
    "lean on its vocabulary or cite it for context stays 'weak' no matter how often it is cited. "
    "The levels exist to be used: a supporting duty or factor rates 'moderate' rather than "
    "defaulting upward to 'strong', and definitional or framing material rates 'weak'. Weigh the "
    "provisions against each other, then rate each honestly: a set that comes out all "
    "'strong' because nothing was weighed is the calibration failure this scale exists to "
    "prevent, while a small cited set that is genuinely load-bearing throughout is correctly all "
    "'strong'. Ground each statement and its rating ONLY in the content of the Findings citing "
    "that provision — never on the provision's own text, never on other provisions, never on "
    "outside knowledge; the cited set calibrates the scale, never the grounding. Set ref to the "
